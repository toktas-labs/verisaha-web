  ...
  return (
    <section className="bg-brand-off">
      <div className="mx-auto max-w-6xl px-4 py-12 md:py-16">
        <h3 className="mb-6 text-xl font-semibold text-brand-navy md:text-2xl">
          {en ? "Industries We Serve" : "Hizmet Verdiğimiz Sektörler"}
        </h3>
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          {sectors.map(({ title, icon: Icon }) => (
            <div key={title} className="flex items-center gap-4 rounded-2xl border border-brand-navy/30 bg-white p-5">
